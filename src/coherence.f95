...
! This is using the suggestion Dave made about a Partha Mitra method for
! MSC which calculates MSC betweek the k-th and k+1-th tapers
subroutine offTaperMsc(coh, yk1, yk2, nfreq1, nfreq2, k, nOffsets)
  implicit none
  ...
  s1 = sum(abs(yk1(:, 1:(k-1)))**2, 2)
  s2 = sum(abs(yk2(:, 2:k))**2, 2)

  do i = 1, nOffsets
    cs(:) = sum( yk1(:, 1:(k-1)) * conjg(yk2(i:(i+nfreq1-1), 2:k)), 2 )
    coh(i, :) = realpart(cs(:))**2 + imagpart(cs(:))**2
    coh(i, :) = coh(i, :) / ( s1(:) * s2(i:(i+nfreq1-1)) )
  end do

end subroutine offTaperMsc
